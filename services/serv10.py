...
def gestion_fecha_devolucion( id_prestamo, nueva_fecha):
    ...
    try:
        ...
        if prestamo is not None: 
            ...
            c.execute('''SELECT id fecha FROM fechasDevolucion WHERE idreserva = %s ''',(prestamo[0]))
            conn.commit()
            fechas_devolucion = c.fetchone()

            if fechas_devolucion: 
                #Update de la fecha de prestamo
                c.execute('''UPDATE fechasDevolucion SET fechadevolucion = %s WHERE id = %s''', (fechas_devolucion[0],nueva_fecha) )
                conn.commit()
                conn.close()
                return True
            else:
